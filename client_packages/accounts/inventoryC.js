...
//         return;
//     mp.events.callRemote('checkflyy');
// });
const controlsIds = {
    F5: 327,
    W: 32, // 232
...
// mp.events.add('renderr', () => {
    mp.events.add('render', () => {
        if (!mp.players.local.getVariable('isAdminLogged' )) return;
        ...
        if (controls.isControlJustPressed(0, controlsIds.F5)) {
            ...
            mp.events.callRemote('server::setInvisible', fly.flying);
        } else if (fly.flying) {
            let updated = false;
            const position = mp.players.local.position;
            ...
            if (controls.isControlPressed(0, controlsIds.W)) {
                if (fly.f < 3.0) { fly.f *= 1.015; } // Adjust the multiplier
...
//     //         inv = mp.browsers.new('package://accounts/cef/inv/index.html');
//     //         //auto = true;                                                          mp.gui.chat.push("[Отладка]: Вы получили броник");
//     //     };
// }); 
